...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-4 rounded-3xl border border-rule-table bg-card p-5 text-card-foreground shadow-sm",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("font-title text-[15px] leading-none font-bold", className)}
      {...props}
    />
  )
}
